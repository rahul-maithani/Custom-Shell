...
from notNeeded.filemang_gui import Ui_MainWindow  # Your PyQt5-designed GUI
from notNeeded.backend import shellBackend        # Logic from filemang.py
import sys
...
class FileManQtWindow(QtWidgets.QMainWindow):
    ...
    def create_file(self):
        name, ok = QtWidgets.QInputDialog.getText(self, 'Create File', 'Enter file name:')
        if ok and name:
            result = self.backend.createnewFile(name)
            msg = "File created successfully." if result == 0 else "Failed to create file."
            QtWidgets.QMessageBox.information(self, "Create File", msg)

    def create_folder(self):
        name, ok = QtWidgets.QInputDialog.getText(self, 'Create Folder', 'Enter folder name:')
        if ok and name:
            result = self.backend.createNewFolder(name)
            msg = "Folder created successfully." if result == 0 else "Failed to create folder."
            QtWidgets.QMessageBox.information(self, "Create Folder", msg)

    def delete_file(self):
        name, ok = QtWidgets.QInputDialog.getText(self, 'Delete File', 'Enter file name:')
        if ok and name:
            result = self.backend.deleteFile(name)
            msg = "File deleted successfully." if result == 0 else "Failed to delete file."
            QtWidgets.QMessageBox.information(self, "Delete File", msg)

    def delete_folder(self):
        name, ok = QtWidgets.QInputDialog.getText(self, 'Delete Folder', 'Enter folder name:')
        if ok and name:
            result = self.backend.deleteFolder(name)
            msg = "Folder deleted successfully." if result == 0 else "Failed to delete folder."
